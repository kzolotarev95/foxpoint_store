...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(`${getApiBaseUrl()}/api/me/notifications/clear`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    return redirectToTarget(
      ...
      returnTo,
      "error",
      await parseApiError(response, "Не удалось очистить уведомления.")
    );
  }

  return redirectToTarget(request, returnTo, "success", "Уведомления очищены.");
}
